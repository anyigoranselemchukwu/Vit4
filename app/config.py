...
THE_ODDS_API_KEY: str      = get_env("THE_ODDS_API_KEY",      "")
...
def print_config_status() -> None:
    ...
    _UNSAFE_KEY = "vit-change-this-in-production-fallback-key"
    using_fallback_key = JWT_SECRET_KEY == _UNSAFE_KEY or SECRET_KEY == _UNSAFE_KEY

    print(f"\n{'='*55}")
    print(f"  VIT Sports Intelligence Network v{APP_VERSION}")
    print(f"{'='*55}")
    print(f"  {'✅' if not using_fallback_key else '⚠️ '} JWT/Secret Key:     {'Configured' if not using_fallback_key else 'USING FALLBACK — set JWT_SECRET_KEY'}")
