...
def build_prediction_response(prediction: Prediction, match: Match, orchestrator: Optional[object] = None) -> PredictionResponse:
...
async def predict(
    ...
    try:
        ...
        # E2 + E4 — run through Module E orchestrator (writes audit log)
        try:
            from app.modules.ai.orchestrator import generate_ai_prediction
            raw_result = await generate_ai_prediction(
                features, idempotency_key, orchestrator, db, triggered_by="api"
            )
        except Exception:
            # Fallback to direct orchestrator call if Module E unavailable
        ...
        # FIX: When there is no positive-edge bet, determine_best_bet returns
        # {"odds": 0, ...}.  Saving entry_odds=0 corrupts CLV and display data.
        # Fall back to the relevant side's market odds so the record is valid.
        best_odds_val = best_bet.get("odds") or 0
        if best_odds_val <= 0:
            side_fallback = best_bet.get("best_side") or max(
                {"home": home_prob, "draw": draw_prob, "away": away_prob},
                key=lambda k: {"home": home_prob, "draw": draw_prob, "away": away_prob}[k]
            )
            best_odds_val = {"home": home_odds, "draw": draw_odds, "away": away_odds}.get(
                side_fallback, home_odds
            )
        ...
        return build_prediction_response(prediction, db_match, orchestrator)
...
async def get_match_insights(
    ...
    """
    Generate Google Gemini AI tactical insights for a specific prediction.
    Returns gracefully when GEMINI_API_KEY is not set.
    """
    from app.db.models import Match, Prediction
    ...
    insights = await generate_match_insights(
        ...
        edge=pred.vig_free_edge or 0.0,
        entry_odds=pred.entry_odds,
        confidence=pred.confidence if isinstance(pred.confidence, float) else 0.5,
    )
    return {"match_id": match_id, **insights}
